...
The following example writes the `iris` data 100 times to an in-memory SQLite database. Profiling data are collected at both R and native levels, native stack traces are commingled with the R stack traces where appropriate.
The results are written (to `Rprof.out` by default) in an `Rprof()`-compatible data format, which can be consumed by `profvis` and other existing packages.
