...
# jointprof is looking for calls to .Call, but Rcpp creates .Primitive(".Call")
# Replacing.
code[[1]] <- quote(.Call)
...
The *jointprof* package currently requires Linux, it has been tested on Ubuntu only. Various system dependencies are required:

- *jointprof* requires [perftools](https://github.com/gperftools/gperftools)
- the necessary *RProtoBuf* package requires ProtoBuf development files
...
Installing the R package from GitHub will also install two dependencies from GitHub, [*pprof*](https://r-prof.github.io/r-pprof/index.html) and [*profile*](https://r-prof.github.io/profile/index.html), as well as all other required R packages:
...
```{r minimal-graph}
dir.create(knit_dir("guide_fig"), recursive = TRUE)
svg_file <- knit_dir("guide_fig/minimal.svg")
...
png_file <- knit_dir("guide_fig/minimal.png")
rsvg::rsvg_png(svg_file, png_file, width = 680)
```

[![Call graph (click to show image)](guide_fig/minimal.png)](guide_fig/minimal.svg)
...
```sh
pprof -http localhost:8080 jointprof.pb.gz
```
